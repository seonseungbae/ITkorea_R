#1. 데이터 수집 : 기상청 데이터를 불러와서 실습한 것임.
#1-1 수집한 데이터 불러오기   # 날씨데이터 ===> 이름만 변경해주면됨.
...
#2. 데이터구조와 변수 확인(int chr num)
# int : 정수,  num : 실수(소수점까지)  chr : 문자

str(weather)   # 각 컬럼(벡터)들 데이터 타입 꼭 확인

# head로 상위데이터 확인
print(head(weather))    # 상위 6개항 데이터 확인(head = 기본이 6개가 나옴) 
print(nrow(weather))    # 8208 개  = 내가 수집한 데이터 전체 개수
print(summary(weather))   # 전체 컬럼 요약(사분위수, 평균)이 된다.
print(table(weather$지점명))   # 대전 4104개 서울 4104개 각 지점명 개수확인.

# 컬럼이 한글입니다.  실무에서는 한글을 쓰지 않는다.(한글 용량이 2배크기 때문) 
# 컬럼명을 한글에서 영어로 변경하기
# col + names 
colnames(weather) = c('station_id', 'station_name', 'datatime', 'temp', 
                      'precip', 'windspeed', 'winddir', 'humidity', 'CA')
# 한글 변수명은 일부 함수(기능)에서 오류로 인식. 즉, 오류 발생가능성 증가

print(head(weather))  # 컬럼이 영어로 변경되었는지 확인

# 일시(datatime) 문자에서 data 형태로 형 변환
# as.Date(연, 월, 일만 가능했다.)  --> 시간 정보를 무시한다.
# as.POSIXct(포직스) --> 날짜와 시간을 모두 형 변환

weather$datatime = as.POSIXct(weather$datatime, format = '%Y-%m-%d %H:%M')

str(weather)  # 날짜와 시간 모두 형 변환 확인(위의 코드 공식)

# 누락된 데이터 결측치(Missing Value가 있는지 없는지 확인
# col + Sums(총합)                # 전체 컬럼 NA 개수 통계
print(colSums(is.na(weather)))   # precip 강수량이 결측치가 많다. 7364개

# 결측값을 0 으로 대체하거나 해당 컬럼의 평균으로 대체하기

# 결측값을 0으로 대체
# weather$precip[is.na(weather$precip)] = 0

# ifelse,평균으로 대체할 경우 0 을 mean 으로 바꿔주면 된다.
...
# 히스토그램(단일변수 시각화)
par(mfrow = c(1,1)) # 1행 3열로 그래프 배치(단, 기본그래프만 가능)
...
# IQR을 이용해서 이상치 판단(6월 22일 배운것임. 처음 배움) = 이상치 제거(이걸로)
...
# sum : 총합
print(sum(is.na(weather$temp)))  # 이상치 데이터가 0 이나왔음.(이상한데이터제거)

cat('이상치 데이터 개수 : ', sum(is.na(weather$temp)), '\n')

# 5월 1일부터 ~ 5월 31일까지 대전지점 데이터 필터링
# 대전 5월 날씨 가져옴.
...
forecasted = forecast(auto_model, h = 30)    # 앞으로 30시간을 시계열 예측
